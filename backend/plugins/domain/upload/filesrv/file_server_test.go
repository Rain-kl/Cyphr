...
func TestServeFileRangeSupport(t *testing.T) {
	...
	// Deterministic 1024-byte payload standing in for audio content.
	payload := make([]byte, 1024)
	for i := range payload {
		payload[i] = byte(i % 251)
	}
	if err := os.WriteFile(filepath.Join(tempDir, "range.mp3"), payload, 0o644); err != nil {
		t.Fatalf("failed to write range test file: %v", err)
	}
	...
	uploadRecord := models.Upload{
		...
		MimeType:   "audio/mpeg",
		Extension:  "mp3",
		Type:       "avatar", // Whitelisted by default: reachable without auth.
		Status:     models.UploadStatusUsed,
		AccessMode: 1,
	}
	if err := dbConn.Create(&uploadRecord).Error; err != nil {
		t.Fatalf("failed to create upload record: %v", err)
	}
	...
	serve := func(rangeHeader string) *httptest.ResponseRecorder {
		req, _ := http.NewRequest("GET", "/f/9001", nil)
		if rangeHeader != "" {
	...
	t.Run("full content without Range returns 200 with Accept-Ranges", func(t *testing.T) {
		w := serve("")
		if w.Code != http.StatusOK {
			t.Errorf("GET /f/9001 status = %d, want %d", w.Code, http.StatusOK)
		}
		if got := w.Header().Get("Accept-Ranges"); got != "bytes" {
			t.Errorf("GET /f/9001 Accept-Ranges = %q, want %q", got, "bytes")
		}
		if !bytes.Equal(w.Body.Bytes(), payload) {
			t.Errorf("GET /f/9001 body length = %d, want %d (full content)", w.Body.Len(), len(payload))
		}
	})

	t.Run("bounded range returns 206 with Content-Range", func(t *testing.T) {
		w := serve("bytes=0-99")
		if w.Code != http.StatusPartialContent {
			t.Errorf("GET /f/9001 Range bytes=0-99 status = %d, want %d", w.Code, http.StatusPartialContent)
		}
		if got, want := w.Header().Get("Content-Range"), "bytes 0-99/1024"; got != want {
			t.Errorf("GET /f/9001 Content-Range = %q, want %q", got, want)
		}
		if !bytes.Equal(w.Body.Bytes(), payload[:100]) {
			t.Errorf("GET /f/9001 Range bytes=0-99 body length = %d, want %d", w.Body.Len(), 100)
		}
	})

	t.Run("open-ended range returns trailing bytes", func(t *testing.T) {
		w := serve("bytes=1000-")
		if w.Code != http.StatusPartialContent {
			t.Errorf("GET /f/9001 Range bytes=1000- status = %d, want %d", w.Code, http.StatusPartialContent)
		}
		if got, want := w.Header().Get("Content-Range"), "bytes 1000-1023/1024"; got != want {
			t.Errorf("GET /f/9001 Content-Range = %q, want %q", got, want)
		}
		if !bytes.Equal(w.Body.Bytes(), payload[1000:]) {
			t.Errorf("GET /f/9001 Range bytes=1000- body length = %d, want %d", w.Body.Len(), len(payload)-1000)
		}
	})
}
