// Package main is the entrypoint for Cyphr installer CLI and interactive TUI.
package main
...
func handleCLI(paths *config.AppPaths, args []string) {
	...
	case "restart":
		cliRestart(agentSvc)
	case "install":
		cliInstall(paths, agentSvc, args)
	case "update":
		cliUpdate(paths, agentSvc, args)
	case "status":
		cliStatus(agentSvc, modelSvc)
	case "_worker-download":
		cliWorkerDownload(modelSvc, args)
	case "download":
		cliDownload(modelSvc, args)
	case "progress":
		cliProgress(modelSvc)
	case "stop-download":
		cliStopDownload(modelSvc)
	case "models":
		cliModels(modelSvc)
	case "doctor":
...
func cliStart(agentSvc *agent.Service) {
...
func cliStop(agentSvc *agent.Service) {
...
func cliRestart(agentSvc *agent.Service) {
...
func cliInstall(paths *config.AppPaths, agentSvc *agent.Service, args []string) {
	targetDir := paths.AgentDir
	if len(args) >= minDownloadArgs && !strings.HasPrefix(args[1], "-") {
		targetDir = args[1]
	}
	useMirror := true
	skipVenv := false
	...
	err := agentSvc.InstallAgent(opts, func(stage string, _ float64, message string) {
		fmt.Printf("  [%-8s] %s\n", stage, message)
	})
	if err != nil {
		fmt.Printf("✗ 安装失败: %v\n", err)
...
func cliUpdate(paths *config.AppPaths, agentSvc *agent.Service, args []string) {
	target := "all"
	if len(args) >= minDownloadArgs && !strings.HasPrefix(args[1], "-") {
		target = args[1]
	}
	...
	switch target {
	case "agent":
		cliUpdateAgent(paths, agentSvc, useMirror)
	case "installer", "self":
		cliUpdateInstaller(useMirror)
	default:
		cliUpdateBoth(paths, agentSvc, useMirror)
	}
}

func cliUpdateAgent(paths *config.AppPaths, agentSvc *agent.Service, useMirror bool) {
...
func cliUpdateInstaller(useMirror bool) {
	fmt.Println("正在从 GitHub Release 更新 Installer 自身程序...")
	err := updater.UpdateInstaller(defaultRepoOwner, defaultRepoName, useMirror, func(stage string, _ float64, message string) {
		fmt.Printf("  [%-8s] %s\n", stage, message)
	})
	if err != nil {
...
func cliUpdateBoth(paths *config.AppPaths, agentSvc *agent.Service, useMirror bool) {
	...
	_ = agentSvc.InstallAgent(opts, func(stage string, _ float64, message string) {
		fmt.Printf("  [%-8s] %s\n", stage, message)
	})
...
func cliStatus(agentSvc *agent.Service, modelSvc *model.Service) {
	st := agentSvc.Status()
...
func cliWorkerDownload(modelSvc *model.Service, args []string) {
	if len(args) < minWorkerDownloadArgs {
...
func cliDownload(modelSvc *model.Service, args []string) {
	if len(args) < minDownloadArgs {
	...
	modelID := args[1]
	pkgDir := ""
	if len(args) > argIdxPkgDir && !strings.HasPrefix(args[argIdxPkgDir], "-") {
		pkgDir = args[argIdxPkgDir]
	}
	source := "hf"
	if len(args) > argIdxSource {
		source = args[argIdxSource]
	}
	endpoint := ""
	if len(args) > argIdxEndpoint {
		endpoint = args[argIdxEndpoint]
	}
	mode := "bg"
	if len(args) > argIdxMode {
		mode = args[argIdxMode]
	}
...
func cliProgress(modelSvc *model.Service) {
	dst := modelSvc.Status()
...
func cliStopDownload(modelSvc *model.Service) {
...
func cliModels(modelSvc *model.Service) {
